...
The Warp framework uses a few key concepts to allo the construction of
...
Locally, clients can organize their view of remote object hierarchys into
a dynamically adjustable namespace of those object hierarchies.

This allows clients to be developed with an assumed layout of object hierarcies
...
for unique sets of deployment time organication.  A client and a server can
have interposers between them that can perform operations in a manner that
does not violate the client or servers expectaitons.  These interposers may
...
The Warp framework will provide:
   ...
   nspace -- an intelligent gateway dynamic namespace for mouting remote servers
   client frameworks
